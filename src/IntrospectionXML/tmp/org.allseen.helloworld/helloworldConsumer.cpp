...
std::map<alljoyn_interfacedescription, WeakReference*> helloworldConsumer::SourceInterfaces;

helloworldConsumer::helloworldConsumer(AllJoynBusAttachment^ busAttachment)
...
helloworldConsumer::~helloworldConsumer()
{
...
void helloworldConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void helloworldConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void helloworldConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus helloworldConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<helloworldJoinSessionResult^>^ helloworldConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ helloworldWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> helloworldJoinSessionResult^
    {
        auto result = ref new helloworldJoinSessionResult();
        result->Status = AllJoynStatus::Ok;
        result->Consumer = nullptr;

        result->Consumer = ref new helloworldConsumer(watcher->BusAttachment);
...
IAsyncOperation<helloworldMethodOneResult^>^ helloworldConsumer::MethodOneAsync(_In_ int32 interfaceMemberInput)
{
    return create_async([this, interfaceMemberInput]() -> helloworldMethodOneResult^
    {
        ...
        TypeConversionHelpers::SetAllJoynMessageArg(alljoyn_msgarg_array_element(inputs, 0), "i", interfaceMemberInput);
        
        QStatus status = alljoyn_proxybusobject_methodcall(
            ProxyBusObject,
            "org.allseen.helloworld",
            "MethodOne",
            inputs,
            argCount,
            message,
...
IAsyncOperation<helloworldSetPropertyOneResult^>^ helloworldConsumer::SetPropertyOneAsync(_In_ bool value)
{
    return create_async([this, value]() -> helloworldSetPropertyOneResult^
    {
...
IAsyncOperation<helloworldGetPropertyOneResult^>^ helloworldConsumer::GetPropertyOneAsync()
{
    return create_async([this]() -> helloworldGetPropertyOneResult^
    {
        ...
        result->PropertyOne = getContext.GetValue();
        return result;
    });
}

void helloworldConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    UNREFERENCED_PARAMETER(obj);
...
void helloworldConsumer::CallSignalOneSignalHandler(_In_ const alljoyn_interfacedescription_member* member, _In_ alljoyn_message message)
{
    ...
    auto consumer = source->second->Resolve<helloworldConsumer>();
    if (consumer->Signals != nullptr)
    {
        ...
        consumer->Signals->CallSignalOneReceived(consumer->Signals, eventArgs);
    }
}

int32 helloworldConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<helloworldConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<helloworldConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<helloworldConsumer>
    };
    ...
    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "org.allseen.helloworld");
