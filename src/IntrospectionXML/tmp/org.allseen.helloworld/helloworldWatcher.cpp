...
helloworldWatcher::helloworldWatcher(AllJoynBusAttachment^ busAttachment) :
...
helloworldWatcher::~helloworldWatcher()
{
    UnregisterFromBus();
}

void helloworldWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.allseen.helloworld" };
...
void helloworldWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.allseen.helloworld"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.allseen.helloworld"),
            port);
        Added(this, args);
    }
...
void helloworldWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<helloworldWatcher>
        };
...
void helloworldWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_helloworldIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &helloworldWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void helloworldWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void helloworldWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
